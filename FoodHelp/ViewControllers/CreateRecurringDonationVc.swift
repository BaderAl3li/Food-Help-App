...
import FirebaseFirestore
    ...
    final class CreateRecurringDonationVC: UIViewController {
        ...
        @IBAction func finishTapped(_ sender: UIButton) {
            ...
            if scheduleInterval.count > 8 {
                showAlert("Schedule interval must be 8 digits or less.")
                return
            }

            if scheduleTime.count > 15 {
                showAlert("Schedule time format should be short (example: 14:30).")
                return
            }

            if quantityText.count > 5 {
                showAlert("Quantity cannot exceed 5 digits.")
                return
            }
